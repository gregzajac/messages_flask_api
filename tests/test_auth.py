...
def test_registration(client):
    response = client.post(
        ...
        json={"username": "testuser", "password": "testuserpwd"},
    )
...
def test_registration_invalid_data(client, data, missing_field):
    ...
    assert msg in response_data["message"][missing_field]


def test_registration_invalid_content_type(client):
    response = client.post(
        "/api/v1/auth/register",
        data={"username": "testuser", "password": "testuserpwd"},
    )
    response_data = response.get_json()
...
def test_registration_already_used_username(client, user):
    response = client.post(
        "/api/v1/auth/register",
        json={"username": user["username"], "password": "testuserpwd"},
    )
    response_data = response.get_json()
