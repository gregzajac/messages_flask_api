...
def test_get_message(client, sample_data):
    ...
    expected_data = {"id": 1, "msg_counter": 1, "msg_text": "Message 1"}
...
def test_get_message_second_time(client, sample_data):
    ...
    expected_data = {"id": 1, "msg_counter": 2, "msg_text": "Message 1"}
...
def test_create_message(client, token, message):
    response = client.post(
        "/api/v1/messages", json=message, headers={"Authorization": f"Bearer {token}"}
    )
    response_data = response.get_json()

    expected_data = {"id": 1, "msg_counter": 0, "msg_text": message["msg_text"]}
...
def test_create_message_invalid_content_type(client, token, message):
    response = client.post(
        "/api/v1/messages", data=message, headers={"Authorization": f"Bearer {token}"}
    )
    response_data = response.get_json()
...
def test_update_message_counter_zero(client, sample_data, token, message):
    ...
    response = client.put(
        "/api/v1/messages/1", json=message, headers={"Authorization": f"Bearer {token}"}
    )
    response_data = response.get_json()
...
def test_update_message_invalid_content_type(client, sample_data, token, message):
    response = client.put(
        "/api/v1/messages/1", data=message, headers={"Authorization": f"Bearer {token}"}
    )
    response_data = response.get_json()
...
def test_update_message_missing_token(client, sample_data, message):
    response = client.put("/api/v1/messages/1", data=message)
    response_data = response.get_json()
...
def test_delete_message(client, sample_data, token):
    response = client.delete(
        "/api/v1/messages/1", headers={"Authorization": f"Bearer {token}"}
    )
    response_data = response.get_json()
...
def test_delete_message_wrong_token(client, sample_data):
    response = client.delete(
        "/api/v1/messages/1", headers={"Authorization": f"Bearer wrong_token"}
    )
    response_data = response.get_json()
